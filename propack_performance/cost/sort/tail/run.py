...
def run_lam(nj, parallel):
    ...
    while i <=num:
        command= "aws lambda invoke --function-name scaling_test_sort --payload \'{\"key1\":\""+str(parallel)+"\"}\' response" +str(i)+".txt" ##
    ...
    filename='response_tail_'+str(nj)+'_'+n_num+'.txt'  ##
    ...
    while i < len(l):
        ...
        i+=1
    scale_time=start_list[int(len(start_list)*0.9)]-start_time ## 
    service_time=end_list[int(len(end_list)*0.9)]-start_time  ##
...
concurrency_list=[5000,4500,4000,3500,3000,2500,2000,1500,1000,500,100]
opt_list=[12,15,12,15,12,15,12,15,12,15,15] ##
